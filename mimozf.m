%% Not correct completely as test cases were incorrectly programmed
...
for iter_1=1:length(EsNo)
    ...
    for iter_2=1:ceil(N/Nt)
        ...
        Rx_data = H * Tx_data + sqrt(Nt*(10^-(EsNo(iter_1) / 10))) * Noise;%
        %% Complex ZF Detector
        % Find the ZF solution
        ZF_sol = inv(H'*H)*H' * Rx_data;  % fill the code
        ...
        H_real= real(H) ; % fill the code
        % Convert received signal to its real equivalent form
        Rx_data_real=  real(Rx_data);  % fill the code
        % Find the ZF solution
        ZF_sol_real = inv(H_real'*H_real)*H_real' * Rx_data_real;  % fill the code
        % Convert it to complex form
        ZF_sol_eqv= inv(H)*Rx_data;  % fill the code
