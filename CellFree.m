        ...
        for m = 1:M
            for k=1:K
             ...
             for kte=1:K
                 num_e = 0;
                 for men = 1:M
                     num_e = num_e + sqrt(Eta(men,kte))*G(men,kte)*Gcap(men,kte);
                 end
                 ...
                 for ked = 1:K
                     if(ked~=kte)
                         den_e_i = 0;
                         for med = 1:M
                             den_e_i = den_e_i + sqrt(Eta(men,ked))*G(men,kte)*Gcap(men,ked);
                         end
                         den_e1 = den_e1 + (abs(den_e_i))^2;
                     end
                 end

                 Re_k(kte,ite) = log2(1+ ((rho_d_cf*((abs(num_e))^2))/(rho_d_cf*den_e1 + 1)));
